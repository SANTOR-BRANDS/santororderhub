          ...
          <div className="space-y-4 py-4">
            {basketItems.map((item) => (
              <div key={item.id} className="border rounded-lg p-4">
                ...
                {/* --- ITEM VISUAL DETAILS RESTORED HERE --- */}
                <div className="text-xs text-muted-foreground space-y-1 mb-3">
                  {/* COMBO HANDLING */}
                  {item.isCombo && item.combo2 ? (
                    <>
                      <div className="font-medium mt-1 text-primary">{t('basket.dish')} 1:</div>
                      ...
                      {item.dish.category !== 'DRINKS' && item.dish.category !== 'FRESH SALMON' && item.dish.category !== 'DESSERT' && (
                        <div>• {t('basket.sauce')}: {(() => {
                          const SAUCES = getSaucesByRestaurant(item.dish.restaurant);
                          const sauceIds = item.sauce.split(', ').filter(id => id);
                          if (sauceIds.length === 0) return t('basket.noSauce');
                          return sauceIds.map(id => {
                            const sauce = SAUCES.find(s => s.id === id);
                            return sauce ? (t(sauce.id) || sauce.name) : '';
                          }).filter(Boolean).join(', ');
                        })()}</div>
                      )}
                      {/* Dish 1 Addons */}
                      {item.addOns.length > 0 && (
                        <div>• {t('basket.addOns')}: {item.addOns.map(a => t(a.id) || a.name).join(', ')}</div>
                      )}
                      {/* Dish 1 Extras */}
                      {item.extraPls && item.extraPls.length > 0 && (
                        <div>• {t('basket.extra')}: {item.extraPls.map(ex => {
                          if (ex.isIncremental && item.incrementalExtras) {
                             const qty = item.incrementalExtras.get(ex.id) || 0;
                             if (qty > 0) {
                               const totalGrams = (ex.incrementalUnit || 20) * qty;
                               return `${t(ex.id) || ex.name} (${totalGrams}g)`;
                             }
                          }
                          return t(ex.id) || ex.name;
                        }).filter(Boolean).join(', ')}</div>
                      )}

                      <div className="font-medium mt-2 text-primary">{t('basket.dish')} 2:</div>
                      {item.combo2.selectedVariant && (
                        <div>• {t('basket.variation')}: {t(item.combo2.selectedVariant.id) === item.combo2.selectedVariant.id ? item.combo2.selectedVariant.name : t(item.combo2.selectedVariant.id)}</div>
                      )}
                      {item.combo2.spicyLevel !== undefined && (
                        <div>• {t('basket.spicyLevel')}: {item.combo2.spicyLevel}</div>
                      )}
                      {item.dish.category !== 'DRINKS' && item.dish.category !== 'FRESH SALMON' && item.dish.category !== 'DESSERT' && (
                        <div>• {t('basket.sauce')}: {(() => {
                          const SAUCES = getSaucesByRestaurant(item.dish.restaurant);
                      ...
                      {item.combo2.addOns.length > 0 && (
                        <div>• {t('basket.addOns')}: {item.combo2.addOns.map(a => t(a.id) || a.name).join(', ')}</div>
                      )}
                      {/* Dish 2 Extras */}
                      {item.combo2.extraPls && item.combo2.extraPls.length > 0 && (
                        <div>• {t('basket.extra')}: {item.combo2.extraPls.map(ex => {
                          if (ex.isIncremental && item.combo2!.incrementalExtras) {
                             const qty = item.combo2!.incrementalExtras.get(ex.id) || 0;
                             if (qty > 0) {
                               const totalGrams = (ex.incrementalUnit || 20) * qty;
                               return `${t(ex.id) || ex.name} (${totalGrams}g)`;
                             }
                  ...
                  {/* Cutlery - shared for both combo and single */}
                  <div className="pt-1">{t('basket.cutlery')}: {item.needsCutlery ? t('basket.yes') : t('basket.no')}</div>
                </div>
                {/* --- END RESTORED DETAILS --- */}
