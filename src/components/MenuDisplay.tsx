...
const MenuDisplay = ({
  ...
  return <div className={cn('min-h-screen pb-20', theme.bg)}>
      <div className="container mx-auto px-4 py-6">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">Choose your favorite dishes and customize them to your taste!</p>
        </div>
        ...
        <div className={cn('rounded-lg mb-6 p-6 text-center', restaurant === 'restory' ? 'bg-gradient-restory text-restory-foreground' : 'bg-gradient-nirvana text-nirvana-foreground')}>
          ...
          <p className="opacity-90">
            {restaurant === 'restory' ? 'Try our signature Tonkatsu Don - perfectly crispy and delicious!' : 'Fresh Grilled Beef on Rice - tender and flavorful!'}
          </p>
        </div>
        ...
        <ScrollArea className="mb-6">
          <div className="flex gap-2 pb-2">
            {categories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(category)} className={cn('whitespace-nowrap transition-smooth', selectedCategory === category ? theme.selectedButton : `border-${theme.accent}/20 hover:border-${theme.accent}/40`)}>
                {category}
              </Button>)}
          </div>
        ...
        {/* Dishes Grid */}
        {filteredDishes.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
