      ...
      <div className="container mx-auto px-4 py-4">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap justify-center gap-2">
            {restaurants.map(restaurant => <button key={restaurant.id} onClick={() => restaurant.available && onRestaurantChange(restaurant.id)} disabled={!restaurant.available} className={cn('px-4 py-2 rounded-lg font-medium transition-smooth', 'border border-white/20 backdrop-blur-sm', restaurant.available ? selectedRestaurant === restaurant.id ? 'bg-white/20 border-white/40 shadow-card' : 'hover:bg-white/10 hover:border-white/30' : 'opacity-40 cursor-not-allowed bg-white/5')}>
                {restaurant.name}
              </button>)}
          </div>
        </div>
