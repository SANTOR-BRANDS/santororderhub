...
// Get all menus combined for unified food court view
export const getAllMenus = (): Dish[] => {
  const allDishes: Dish[] = [
    ...restoryMenu.map(dish => ({ ...dish, image: getDishImage(dish.id) || dish.image })),
    ...nirvanaMenu.map(dish => ({ ...dish, image: getDishImage(dish.id) || dish.image })),
    ...mejaiMenu.map(dish => ({ ...dish, image: getDishImage(dish.id) || dish.image })),
  ];
  return allDishes;
};
...
export const getAllCategories = (): string[] => {
  const allDishes = getAllMenus();
  const categories = [...new Set(allDishes.map(dish => dish.category))];
  return ['ALL', ...categories];
};

// Get featured/popular dishes for the "Mix & Match Favorites" section
export const getFeaturedDishes = (): Dish[] => {
  const allDishes = getAllMenus();
  // Return dishes with isSpecial flag or specific popular items
  const featured = allDishes.filter(dish => 
    dish.isSpecial || 
    dish.id.includes('COM') || // Combo deals
    dish.name.includes('⭐') || 
    dish.name.includes('🔥') ||
    dish.name.includes('👍')
  );
  
  // If not enough featured, add some from each category
  if (featured.length < 12) {
    const additional = allDishes
      .filter(d => !featured.includes(d) && d.isAvailable !== false)
      .slice(0, 12 - featured.length);
    return [...featured, ...additional];
  }
  
  return featured.slice(0, 16); // Limit to 16 items
};
